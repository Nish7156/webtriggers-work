...
if (typeof Object.create !== "function") {
    ...
    var b = {
        init: function(g, f) {
            this.options = a.extend({}, a.fn.singlePageNav.defaults, g);
            this.container = f;
            this.$container = a(f);
        ...
        handleClick: function(g) {
            ...
            if (h.length) {
                i.clearTimer();
                if (typeof i.options.beforeStart === "function") {
                    i.options.beforeStart()
                }
                i.setActiveLink(f.hash);
                i.scrollTo(h, function() {
                    if (i.options.updateHash && history.pushState) {
                        history.pushState(null, null, f.hash)
                    }
                    i.setTimer();
                    if (typeof i.options.onComplete === "function") {
                        i.options.onComplete()
                    }
                })
            }
        },
        scrollTo: function(i, g) {
            var h = this;
            var f = h.getCoords(i).top;
            var j = false;
            h.$htmlbody.stop().animate({
                scrollTop: f
            }, {
                duration: h.options.speed,
                easing: h.options.easing,
                complete: function() {
                    if (typeof g === "function" && !j) {
                        g()
                    }
                    j = true
                }
        ...
        setTimer: function() {
            var f = this;
            f.$window.on("scroll.singlePageNav", function() {
                f.didScroll = true
            });
        ...
        checkPosition: function() {
            ...
            var f = this.getCurrentSection(g);
            if (f !== null) {
                this.setActiveLink(f)
            }
        },
        getCoords: function(f) {
            return {
                top: Math.round(f.offset().top) - this.options.offset
            }
        },
        setActiveLink: function(f) {
            var g = this.$container.find("a[href$='" + f + "']");
            if (!g.hasClass(this.options.currentClass)) {
                this.$links.removeClass(this.options.currentClass);
                g.addClass(this.options.currentClass)
            }
        },
        getCurrentSection: function(g) {
            var h, f, i, j;
            for (h = 0; h < this.$links.length; h++) {
                f = this.$links[h].hash;
                if (a(f).length) {
                    i = this.getCoords(a(f));
                    if (g >= i.top - this.options.threshold) {
                        j = f
                    }
                }
            }
            return j || ((this.$links.length === 0) ? (null) : (this.$links[0].hash))
        }
    };
    a.fn.singlePageNav = function(f) {
        return this.each(function() {
